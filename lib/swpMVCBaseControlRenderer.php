...
class swpMVCBaseControlRenderer extends swpMVCBaseModelExtender
{
...
class swpMVCModelControl
{
    ...
    public function option_text($key=false)
    {
        $val = $key !== false ? $key : $this->value;
        if ($this->type !== 'select' or !$this->is_valid()) return '';
        $o = array_flip($this->options);
        return array_key_exists($val, $o) ? $o[$val] : '';
    }
}